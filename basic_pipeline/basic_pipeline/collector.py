...
class Collector_Node(Node):
    ...
    def __init__(self):
        ...
        self.display_result_publisher = self.create_publisher(DisplayResult, self.name + '_display_result', 10, callback_group=self.group1)
    ...
    def camera_callback(self, msg):
        ...
        with self.detect_result_rlock:
            if(self.detect_result_is_init == True and current_frame_id - self.detect_result_frame_id <= self.detect_diff_threshold):
                ...
                #cv2.waitKey(1)
                self.get_logger().info('Current Camera Frame ID: %d | Result Type: %s | Result Frame ID: %d | Result Frame Diff/Delay: %d | Total Time: %f | Server Detect Time: %f' % (current_frame_id, "Detect", self.detect_result_frame_id, current_frame_id - self.detect_result_frame_id, self.detect_result_total_time, self.detect_result_process_time))
                '''
                detect_show_flag = True
                result.method = 0
